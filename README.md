...
- Selectable background: alongside the animated pixel scene there is a static "Night aurora" backdrop — a vector night sky with aurora ribbons tinted by the active theme accent, a starfield with gentle twinkling, a moon and occasional shooting stars over mountain silhouettes. Switching backgrounds cross-fades smoothly, and the heavy pixel renderer is put to sleep while the static mode is active.
