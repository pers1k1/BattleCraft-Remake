...
- The client runs on the Shenandoah collector: its pauses stay in single-digit milliseconds instead of the 200 ms G1 was allowed, and those pauses used to land exactly on Distant Horizons building far terrain. The server keeps G1, where pause length does not affect what the player sees.
