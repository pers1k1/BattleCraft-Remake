...
- Recommended client defaults are versioned and written as soon as the launcher enters the main screen or accepts a new game folder, without requiring the game-settings page to be opened first. A fresh install gets windowed mode, GUI scale 2, music at 2%, and Minecraft accessibility onboarding disabled. The framerate cap follows the refresh rate of the monitor containing the launcher and is synchronized again before each game launch. When the recommended set changes later, the launcher asks once whether to apply it rather than silently overwriting settings the player has tuned. Revision 3 adds these display and sound defaults. Revision 2 clears the pile-up on `N` and `M`: the microphone mute moves to `+`, night vision goggles take `N` and their mode switch `M`, and the bindings that used to share those keys are left unbound.
