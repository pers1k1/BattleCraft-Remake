...
- Recommended client defaults are versioned. A fresh install gets the whole set written before the first launch - windowed mode and automatic GUI scale so the game never opens at a broken size on laptops or high-resolution displays. When the recommended set changes later, the launcher asks once whether to apply it rather than silently overwriting settings the player has tuned, and the same set stays one button away in the game settings. Revision 2 clears the pile-up on `N` and `M`: the microphone mute moves to `+`, night vision goggles take `N` and their mode switch `M`, and the bindings that used to share those keys - the voice chat kill switch and the SecurityCraft camera keys - are left unbound.
